...
------ for city
CREATE PROC sp_product_order_city_wang_city1
@name varchar(20),
@result varchar(20) out
AS
BEGIN
SELECT @result = t3.City
...
exec sp_product_order_city_wang_city1 'Chang', @city out
print @city
END

----for total quantity
CREATE PROC sp_product_order_city_wang_quantity
@name varchar(20),
@rslt int out
AS
BEGIN
SELECT @rslt = t3.TotalQuantity
FROM(SELECT t2.City, t2.ProductID,t2.ProductName, t2.TotalQuantity
     FROM (SELECT t.City, t.ProductID, t.ProductName, t.TotalQuantity, DENSE_RANK() OVER(PARTITION BY ProductID ORDER BY TotalQuantity DESC) rnk
	       FROM (SELECT c.City, p.ProductID, p.ProductName, SUM(od.Quantity) TotalQuantity
		         FROM Customers c JOIN Orders o ON c.CustomerID = o.CustomerID JOIN [Order Details] od ON o.OrderID = od.OrderID Join Products p 
...
BEGIN
declare @rslt int
exec sp_product_order_city_wang_city1 'Chang', @rslt out
print @rslt
END
